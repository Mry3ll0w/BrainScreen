...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------
/// FUNCION PARA OBTENER EL VALOR DE UN BOTON CONCRETO
const getButtonValueHandler = {
    canHandle(handlerInput) {
        return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
            && Alexa.getIntentName(handlerInput.requestEnvelope) === 'GetButtonValue';
    },
    async handle(handlerInput) {
        const sProjectName = Alexa.getSlotValue(handlerInput.requestEnvelope, 'projectName');
        const strButtonLabel = Alexa.getSlotValue(handlerInput.requestEnvelope, 'buttonLabel');
        const userID = handlerInput.requestEnvelope.context.System.user.userId;
        var speakOutput='Intent buttonLabel';
        ...
        try{
            ...
            //Realizamos la peticion para vincular el nombre 
            const response = await getButtonValueFunction(userID,sProjectName, strButtonLabel);
            
            //Comprobamos el estado del boton
            if(response.res ==='true'){
                
                speakOutput= `El estado del boton es encendido`;
                
            }else if(response.res === 'false'){
                
                speakOutput= `El estado del boton es apagado`;
                
            }else{
                
                speakOutput ='Valor res: '+ response.res;
...
// FUNCION PARA OBTENER EL VALOR DE UN BUTON DE UN PROYECTO DADO.
async function getButtonValueFunction(strAmazonUserId,strProjectName, strButtonLabel){
    //Diccionario para el control del acceso al servidor
    let response = null;
    
    try{
        response = await Axios.get(`http://3.210.108.248:3000/buttonValue/${strAmazonUserId}/${strProjectName}/${strButtonLabel}`,
            {
                    "amazonuid": strAmazonUserId,
                    "projectName": strProjectName,
                    "firebaseuid":"",
    ...
    return {res: response.data.res, error:false}
}
